...
export default function EditCommunicationForm({ communication }: EditCommunicationFormProps) {
  ...
  const [formData, setFormData] = useState({
    ...
    congress: communication.congress,
    category: communication.category,
    authors: communication.authors,
    files: Object.entries(communication.files).reduce((acc: any, [key, value]: [string, any]) => {
      // Convertir los objetos de archivo a File (simulado)
      acc[key] = new File([""], value.name, {
        type: key === "video" ? "video/mp4" : "application/pdf",
      })
      return acc
    }, {}),
  })
  ...
  const handleSaveDraft = async () => {
    ...
    try {
      ...
      router.push(`/plataforma/comunicaciones/${communication.id}`)
    } catch (error) {
  ...
  const coauthors = formData.authors.filter((author: any) => author.role === "coautor")
